...
Hey, BUILDNANA Dev! Add your new changes in [unreleased] heading, below one of
these heading:
...
- RNG can now be set and seeded on init (as `BUILDNANAOpt.rng`) and in runtime
  (#1097) - @Stanko
...
- Added topMostOnlyActivate buildnana option. When true, only the topmost object
...
- Added `BUILDNANAOpt.defaultLifetimeScope` for setting the default lifetime
  scope used for event handlers - @lajbel
...
- Added `BUILDNANAOpt.types`, `buildnanaTypes()` and `Opt` to config specific
...
- Now, you can use `color(c)` with a hexadecimal literal number (ex: 0x00ff00) -
  ...
  ```
- **(!)** `BUILDNANACtx` doesn't use generics anymore. Now, `BUILDNANACtxT` uses
  them - @lajbel
- Now, `buildnana` will return `BUILDNANACtx` or `BUILDNANACtxT` depending if
  it's using Advanced TypeScript Features or not - @lajbel
...
- The debug `record()` function now records with sound enabled like it should -
  @dragoncoder047
- Now `BUILDNANAOpt.spriteAtlasPadding` is set to `2` by default - @lajbel
...
- Added `throwError()` for throwing custom errors to the blue screen, even
  errors BUILDNANA can't handle. - @lajbel
...
- (**!**) Renamed `BUILDNANAOpt.tagsAsComponents` to
  `BUILDNANAOpt.tagComponentIds` - @lajbel
...
- Fixed sprite scaling not working properly with `BUILDNANAOpt.letterbox` -
  @mflerackers
...
- **(!)** Layers now work globally, no longer only between siblings -
  @mflerackers
- **(!)**: Changed default behavior to `buildnana({ tagsAsComponents: false })`
...
- Added `buildnana({ sprit`e`AtlasPadding })` for setting the space between the
...
- Added tags and components separation in `BUILDNANAOpt.tagsAsComponents`
...
- Added `.use()`, `.unuse()` and `.has()` to `GameObjRaw`, to add, remove and
  check components. This only works with `BUILDNANAOpt.tagsAsComponents` set to
  `true`
...
- Added the `animate()` component to _animate_ the properties of an object using
  keyframes. Check out
  [Animation Example](https://play.buildnana.com/?example=animation)
...
- Added `BUILDNANAOpt.debugKey` for customizing the key used to toggle debug
  mode
...
- Deprecated `kaboom()` in favor of `buildnana()` (you can still use `kaboom*`)
...
- **(!)** Removed compatibility to use two BUILDNANA frames in the same page
