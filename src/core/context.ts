...
import { BlendMode, type BUILDNANAPlugin } from "../types";
import {
    download,
    downloadBlob,
    downloadJSON,
...
// The context is the way the user interact with a BUILDNANA game.
export const createContext = (
    e: Engine,
    plugins?: BUILDNANAPlugin<Record<string, unknown>>[],
    ...
    const ctx: BUILDNANACtx = {
        _k: e,
        // @ts-ignore
        VERSION: BUILDNANA_VERSION,
        // asset load
        loadRoot,
        loadProgress,
        loadSprite,
    ...
    if (exportToGlobal) {
        for (const key in ctx) {
            ((window as any)[key]) = ctx[key as keyof BUILDNANACtx];
        }
    }
