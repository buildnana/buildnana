...
export async function genGlobalDTS() {
    ...
    // generate global decls for BUILDNANACtx members
    let globalDts = "";
    let globalDts2 = "";

    const imp =
        "import { BUILDNANACtx, default as BUILDNANA } from \"../doc\"\n";
    const imp2 = "type BUILDNANA = typeof buildnana;";

    globalDts += "declare global {\n";

    for (const stmt of stmts) {
        if (stmt.name === "BUILDNANACtx") {
            if (stmt.kind !== "InterfaceDeclaration") {
                throw new Error("BUILDNANACtx must be an interface.");
            }
            for (const name in stmt.members) {
                globalDts += `\tconst ${name}: BUILDNANACtx["${name}"]\n`;
            }
    ...
    globalDts2 += "const buildnana: BUILDNANA;\n";
    globalDts2 += "const kaboom: BUILDNANA;\n";
    globalDts += `\tconst buildnana: typeof BUILDNANA\n`;
    globalDts += `\tconst kaboom: typeof BUILDNANA\n`;
    ...
    if (!globalGenerated) {
        throw new Error(
            "BUILDNANACtx not found, failed to generate global defs.",
        );
    }
